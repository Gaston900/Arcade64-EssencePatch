...
void cps2_state::cps1_cps_a_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
...
uint16_t cps2_state::cps1_cps_b_r(offs_t offset)
{
...
void cps2_state::cps1_cps_b_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
...
void cps2_state::cps1_gfxram_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
...
void cps2_state::cps1_build_palette( const uint16_t* const palette_base )
{
	...
	*/
	for (uint8_t page = 0; page < 6; ++page)
	{
		if (BIT(ctrl, page))
		{
			for (uint16_t offset = 0; offset < 0x200; ++offset)
			{
				...
				// HBMAME start
				uint8_t b_adj = 0x0f;
				uint8_t b_div = 0x1e + b_adj;
				uint8_t bright = b_adj + ((palette >> 12) << 1);

				// New code to get rid of grey squares
				uint8_t r = (palette >> 8) & 0x0f;
				uint8_t g = (palette >> 4) & 0x0f;
				uint8_t b = palette & 0x0f;
...
uint32_t cps2_state::screen_update_cps1(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	uint8_t l0 = (layercontrol >> 0x06) & 03;
	uint8_t l1 = (layercontrol >> 0x08) & 03;
	uint8_t l2 = (layercontrol >> 0x0a) & 03;
	uint8_t l3 = (layercontrol >> 0x0c) & 03;
