...
void pgm2_state::mcu_command(bool is_command)
{
	...
	{
//==================== 缘来是你 ========================>>>
        bool hack_enabled = machine().options().bool_value(OPTION_PGM2_MEMCARD_HACK);
        
        if (!hack_enabled && (cmd >= 0xc0 && cmd <= 0xc9))
        {
            m_mcu_regs[3] = (m_mcu_regs[3] & 0xff00ffff) | (0xf4 << 16);
...
#define ORLEG2_INTERNAL_HONGKONG \
    ...
    ROM_REGION( 0x108, "default_card", 0 ) \
    ROM_LOAD( "blank_orleg2_hongkong_card.pg2", 0x000, 0x108, CRC(cff88f98) SHA1(f407a2d77c626587d5e5f2ae74d878763d691182) )
...
#define KOV2NL_INTERNAL_TAIWAN \
    ROM_REGION( 0x04000, "maincpu", 0 ) \
    ROM_LOAD( "gsyx_igs036_taiwan.rom", 0x00000000, 0x0004000, CRC(b3ca3124) SHA1(793d3bdc4bfccb892eb51c351c4ccd103ee9b7ce) ) \
    ROM_REGION( 0x108, "default_card", 0 ) \
    ROM_LOAD( "blank_kov2nl_overseas_card.pg2", 0x000, 0x108, CRC(1155f01f) SHA1(60f7bed1461b362a3da687503cd72ed2d5e96f30) )

#define KOV2NL_INTERNAL_JAPAN \
    ROM_REGION( 0x04000, "maincpu", 0 ) \
    ROM_LOAD( "gsyx_igs036_japan.rom", 0x00000000, 0x0004000, CRC(46344f1a) SHA1(fbe846be4a39e8a4c41417858311faaaebf67cb9) ) \
    ROM_REGION( 0x108, "default_card", 0 ) \
    ROM_LOAD( "blank_gsyx_japan.pg2", 0x000, 0x108, CRC(0d63cb64) SHA1(957cce2d47f3369bc4f98b1652ba8639c08fb9bd) )
...
#define KOV2NL_INTERNAL_HONGKONG \
    ...
    ROM_REGION( 0x108, "default_card", 0 ) \
    ROM_LOAD( "blank_gsyx_hongkong.pg2", 0x000, 0x108, CRC(02842ae8) SHA1(a6cda633b09a706039a79b73db2c258094826f85) )
//======================================================>>>
...
ROM_START( kov2nl_300jp )
    ...
    KOV2NL_PROGRAM_300(kov2nl, a10)
    KOV2NL_VIDEO_SOUND_ROMS
ROM_END

ROM_START( kov2nl_301jp )
    KOV2NL_INTERNAL_JAPAN
    KOV2NL_PROGRAM_301(kov2nl, a10)
    KOV2NL_VIDEO_SOUND_ROMS
ROM_END

ROM_START( kov2nl_302jp )
    KOV2NL_INTERNAL_JAPAN
    KOV2NL_PROGRAM_302(kov2nl, a10)
    KOV2NL_VIDEO_SOUND_ROMS
ROM_END

ROM_START( kov2nl_300tw )
    KOV2NL_INTERNAL_TAIWAN
    KOV2NL_PROGRAM_300(gsyx, tw)
    KOV2NL_VIDEO_SOUND_ROMS
ROM_END

ROM_START( kov2nl_301tw )
    KOV2NL_INTERNAL_TAIWAN
    KOV2NL_PROGRAM_301(gsyx, tw)
    KOV2NL_VIDEO_SOUND_ROMS
ROM_END

ROM_START( kov2nl_302tw )
    KOV2NL_INTERNAL_TAIWAN
    KOV2NL_PROGRAM_302(gsyx, tw)
    KOV2NL_VIDEO_SOUND_ROMS
ROM_END
//======================================================>>>
...
GAME( 2008, kov2nl_300jp, kov2nl, pgm2, pgm2, pgm2_state, init_kov2nl, ROT0, "IGS (Alta license)", "Sangoku-Senki 2 Ranse Eiyuu (V300, Japan)", MACHINE_SUPPORTS_SAVE )
GAME( 2008, kov2nl_301jp, kov2nl, pgm2, pgm2, pgm2_state, init_kov2nl, ROT0, "IGS (Alta license)", "Sangoku-Senki 2 Ranse Eiyuu (V301, Japan)", MACHINE_SUPPORTS_SAVE )
GAME( 2008, kov2nl_302jp, kov2nl, pgm2, pgm2, pgm2_state, init_kov2nl, ROT0, "IGS (Alta license)", "Sangoku-Senki 2 Ranse Eiyuu (V302, Japan)", MACHINE_SUPPORTS_SAVE )

GAME( 2008, kov2nl_300tw, kov2nl, pgm2, pgm2, pgm2_state, init_kov2nl, ROT0, "IGS (Huatong license)", "Sanguo Zhan Ji 2 Gaishi Yingxiong (V300, Taiwan)", MACHINE_SUPPORTS_SAVE )
GAME( 2008, kov2nl_301tw, kov2nl, pgm2, pgm2, pgm2_state, init_kov2nl, ROT0, "IGS (Huatong license)", "Sanguo Zhan Ji 2 Gaishi Yingxiong (V301, Taiwan)", MACHINE_SUPPORTS_SAVE )
GAME( 2008, kov2nl_302tw, kov2nl, pgm2, pgm2, pgm2_state, init_kov2nl, ROT0, "IGS (Huatong license)", "Sanguo Zhan Ji 2 Gaishi Yingxiong (V302, Taiwan)", MACHINE_SUPPORTS_SAVE )

GAME( 2007, orleg2_101hk, orleg2, pgm2, pgm2, pgm2_state, init_orleg2, ROT0, "IGS", "Xiyou Shi E Zhuan 2 (V101, Hong Kong)", MACHINE_SUPPORTS_SAVE )
GAME( 2007, orleg2_103hk, orleg2, pgm2, pgm2, pgm2_state, init_orleg2, ROT0, "IGS", "Xiyou Shi E Zhuan 2 (V103, Hong Kong)", MACHINE_SUPPORTS_SAVE )
GAME( 2007, orleg2_104hk, orleg2, pgm2, pgm2, pgm2_state, init_orleg2, ROT0, "IGS", "Xiyou Shi E Zhuan 2 (V104, Hong Kong)", MACHINE_SUPPORTS_SAVE )

GAME( 2007, orleg2_101tw, orleg2, pgm2, pgm2, pgm2_state, init_orleg2, ROT0, "IGS", "Xiyou Shi E Zhuan 2 (V101, Taiwan)", MACHINE_SUPPORTS_SAVE )
GAME( 2007, orleg2_103tw, orleg2, pgm2, pgm2, pgm2_state, init_orleg2, ROT0, "IGS", "Xiyou Shi E Zhuan 2 (V103, Taiwan)", MACHINE_SUPPORTS_SAVE )
GAME( 2007, orleg2_104tw, orleg2, pgm2, pgm2, pgm2_state, init_orleg2, ROT0, "IGS", "Xiyou Shi E Zhuan 2 (V104, Taiwan)", MACHINE_SUPPORTS_SAVE )
//======================================================>>>
