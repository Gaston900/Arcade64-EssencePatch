...
uint16_t neogeo_state::neogeo_paletteram_r(offs_t offset)
{
	return m_paletteram[m_palette_bank + offset];
}


void neogeo_state::neogeo_paletteram_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
...
uint16_t neogeo_state::neogeo_video_register_r(address_space &space, offs_t offset, uint16_t mem_mask)
{
	uint16_t  ret = 0U;
...
void neogeo_state::neogeo_video_register_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
	/* accessing the LSB only is not mapped */
	if (mem_mask != 0x00ff)
	{
