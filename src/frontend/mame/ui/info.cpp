...
#endif
		// 获取中文名称
		const char* chinese_desc = m_machine.options().value("chinesename");
		std::string display_desc;
		if (chinese_desc && *chinese_desc)
			display_desc = chinese_desc;
		else
			display_desc = system_list::instance().systems()[driver_list::find(m_machine.system().name)].description;
