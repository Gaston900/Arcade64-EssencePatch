...
public:
	...
	void kof10thBankswitch(uint16_t nBank);
	uint16_t kof10th_RAM2_r(offs_t offset);
	uint16_t kof10th_RAMB_r(offs_t offset);
	void kof10th_custom_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0);
	void kof10th_bankswitch_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0);
	...
	uint16_t kof2003_r(offs_t offset);
    ...
    void neogeo_darksoft_cx_decrypt(uint8_t*sprrom, uint32_t sprrom_size);

	uint16_t m_cartridge_ram[0x1000]; // bootlegs

	// for kof10th
	uint8_t* m_mainrom = nullptr;
	uint8_t* m_fixedrom = nullptr;
...
public:
	...
	uint16_t fatfury2_protection_16_r(offs_t offset);
	void fatfury2_protection_16_w(offs_t offset, uint16_t data);
...
public:
	...
	void kof98_prot_w(uint16_t data);
	uint16_t kof98_prot_r(offs_t offset);
...
public:
	...
	void mslugx_protection_16_w(offs_t offset, uint16_t data);
	uint16_t mslugx_protection_16_r(address_space &space, offs_t offset);
	void mslugx_install_protection(cpu_device* maincpu);

	uint16_t     m_mslugx_counter = 0U;
...
public:
	...
	uint16_t pvc_prot_r(offs_t offset);
	void pvc_prot_w(offs_t offset, uint16_t data, uint16_t mem_mask = ~0);
	...
	void mslug5nd_decrypt_68k(uint8_t* rom, uint32_t size);
...
public:
	...
	void sbp_lowerrom_w(offs_t offset, uint16_t data);
...
public:
	...
	void mslug3a_bankswitch_w(uint16_t data);
	void kof2000_bankswitch_w(uint16_t data);
	uint16_t prot_9a37_r();
	uint16_t sma_random_r();
	void reset_sma_rng();
	...
	void mslug3cqt_decrypt_68k(uint8_t* base);
